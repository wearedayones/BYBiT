...
class PromotionManager:
    ...
    async def evaluate(self) -> bool:
        ...
        if not ag or ag.get("env") == "mainnet":
            return False
        ...
        log.info("Promotion evaluation",
                 ...
                 max_drawdown=max_drawdown, criteria=criteria)
        ...
        if (
            ...
            await self._db.execute(
                "UPDATE agent_state SET env = 'mainnet', last_promoted_at = now(), updated_at = now() WHERE id = 'singleton'"
            )
            log.info("PROMOTED TO MAINNET", win_rate=win_rate, sharpe=sharpe)
            return True
